...
public class Enemy {
	...
	public boolean canMove(int[][] walls, int tileSize, int x2, int y2) {
		...
		// bounds check
		if (topTile < 0 || bottomTile >= walls.length ||
			leftTile < 0 || rightTile >= walls[0].length) {
			return false;
		}
		if (walls[topTile][leftTile] == 0 ||
			walls[topTile][rightTile] == 0 ||
			walls[bottomTile][leftTile] == 0 ||
	...
	public void updateEnemy(int[][] walls, int tileSize) {
		...
		int stepY = 0;
		if (dx > 0) stepX = speed;
		else if (dx < 0) stepX = -speed;
		if (dy > 0) stepY = speed;
		else if (dy < 0) stepY = -speed;
		boolean moved = false;
